    ...
    <form method="POST" enctype="multipart/form-data" action="{{ Helper::indexUrl() }}update">
        ...
        <div class="px-3 pt-4">
            ...
            <div class="col-{{ $data['col'] }}">
                <div class="row">
                    ...
                    <div class="col-{{ $data['grid'] }}">
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <input type="{{ $form['type'] }}" class="form-control" value="{{ is_array($data['find']) ? $data['find'][$form['field']] : $data['find']->{$form['field']} }}" id="{{ $form['field'] }}" name="{{ $form['field'] }}" @if (isset($form['required']) && $form['required']) required @endif>
                            </div>
                            ...
                            <div class="col-sm-10">
                                <select class="form-control select2" name="{{ $form['field'] }}" @if (isset($form['required']) && $form['required']) required @endif>
                            ...
                            <div class="col-sm-10">
                                <input type="{{ $form['type'] }}" class="form-control" style="padding-top:3px;" id="{{ $form['field'] }}" name="{{ $form['field'] }}[]" @if (isset($form['required']) && $form['required']) required @endif>
                            </div>
                            @else
                            <label for="{{ $form['field'] }}" class="col-sm-2 col-form-label">{{ __('common.' . $form['title']) }}
                                @if (isset($form['required']) && $form['required'])
                ...
                @if(isset($data['find']['timetables']))
                    @foreach(json_decode($data['find']['timetables']) as $key => $value)
                        <div class="row">
                            <label for="title_kh" class="col-sm-2 col-form-label">
                                @if($key === 0) Time & Session  <span class="text-danger">*</span>@endif
                            </label>
                            <div class="col-sm-10">
                                <div class="row mb-3 timetable">
                                    <div class="col-sm-3">
                                        <input type="time" class="form-control" name="times[]" required value="{{ $value->time }}">
                                    </div>
                                    <div class="col-sm-3">
                                        <input type="text" class="form-control" name="sessions[]" required value="{{ $value->session }}">
                                    </div>
                                    <div class="col-sm-4">
                                        <input type="file" class="form-control" name="images[]" style="padding-top:3px;" accept="image/*">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card-footer">
            <div class="row">
                ...
                <div class="col-sm-10 d-flex flex-row">
                    ...
                    <button type="submit" class="btn btn-success ml-2" value="{{ Helper::indexUrl() }}" name="save">{{__('common.Save')}}
                    </button>
                </div>
            </div>
        </div>
